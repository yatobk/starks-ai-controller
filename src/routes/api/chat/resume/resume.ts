...
export const ResumeChat = async (app: FastifyInstance, options, done) => {
    app.get(`${baseApiRoute}/chat/resume`, { preHandler: [Middleware] }, async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        try {
            ...
            const chat = await updateSession(id, 'active');
            if (!chat) {
